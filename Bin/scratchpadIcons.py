...
def find_browser_key_from_title(title, config_keys):
    """
...
def main():
    ...
    for win in windows:
        ...
        # Decide whether to use class match or browser-title match
        if app_class in BROWSER_CLASSES:
            key = find_browser_key_from_title(title, config.keys())
        else:
            # Non-browser → class match
